...
describe("SftpProvider", () => {
  ...
  beforeEach(() => {
    ...
    mockMkdir.mockImplementation((path, callback) => callback(null));
    mockStat.mockImplementation((path, callback) => callback(null, { isDirectory: () => true }));
    ...
    mockReaddir.mockImplementation((path, callback) => callback(null, []));
    mockUnlink.mockImplementation((path, callback) => callback(null));
    mockRmdir.mockImplementation((path, callback) => callback(null));
  });
  ...
  describe("Initialization", () => {
    it("should establish SSH connection and SFTP subsystem", async () => {
      ...
      expect(mockConnect).toHaveBeenCalledWith(expect.objectContaining({ host: "127.0.0.1" }));
      expect(mockSftp).toHaveBeenCalled();
      expect(mockMkdir).toHaveBeenCalledWith(expect.stringContaining("/backups"), expect.any(Function));
    });
    ...
    it("should throw error if SSH connection fails", async () => {
      ...
      const provider = new SftpProvider(defaultOptions);
      // @ts-ignore
      await expect(provider._initialize()).rejects.toThrow(/SSH connection failed/);
    });
  ...
  describe("Streaming Uploads", () => {
    ...
    it("should create a BSON write stream and nested directories", async () => {
      ...
      expect(mockMkdir).toHaveBeenCalledWith(expect.stringContaining("db1/"), expect.any(Function));
      expect(mockCreateWriteStream).toHaveBeenCalledWith(expect.stringContaining("db1/col1.bson"));
    });

    it("should apply gzip compression if compress is true", async () => {
      const gzipProvider = new SftpProvider({ ...defaultOptions, compress: true });
      // @ts-ignore
      ...
      expect(mockCreateWriteStream).toHaveBeenCalledWith(expect.stringContaining("archive.tar.gz"));
  ...
  describe("Pruning", () => {
    ...
    it("should prune based on maxCount by recursively deleting directories", async () => {
      ...
      const mockDirItem = (name: string) => ({ filename: name, attrs: { isDirectory: () => true } });
      const mockFileItem = (name: string) => ({ filename: name, attrs: { isDirectory: () => false } });

      mockReaddir.mockImplementation((path, callback) => {
        if (path === "/backups/") {
          callback(null, [mockDirItem(run1), mockDirItem(run2), mockDirItem(run3)]);
